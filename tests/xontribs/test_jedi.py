...
def test_jedi_api(jedi_xontrib, jedi_mock, version, prefix, line, start, end,
    ...
    namespaces = [ctx, {'__xonsh__': builtins.__xonsh__}]
    if version == 'new':
...
def test_multiline(jedi_xontrib, jedi_mock, monkeypatch):
    ...
    shell_mock.shell.pt_completer.current_document.text = 'xx = 1\n1 + x'
    ...
    call_args_list = jedi_mock.Interpreter().complete.call_args_list
    assert len(call_args_list) == 1
    call_args = call_args_list[0].args
    assert call_args[0] == 2  # line (one-indexed)
    assert call_args[1] == 5  # column (zero-indexed)
